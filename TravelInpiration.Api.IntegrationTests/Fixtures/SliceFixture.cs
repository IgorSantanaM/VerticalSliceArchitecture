...
public sealed class SliceFixture
{
    ...
    private TravelnspirationDbContext CreateContext(IServiceScope scope)
    {
        var context = scope.ServiceProvider
            .GetRequiredService<TravelnspirationDbContext>();
        return context;
    }

    public async Task ExecuteInTransactionAsync(
        Func<TravelnspirationDbContext, Task> actionToExecute)
    {
        using (_scope = _serviceScopeFactory.CreateScope())
        {
            // FOR THIS SCOPE GIMME FRESH INSTANCES TO USE ON MY METHODS
